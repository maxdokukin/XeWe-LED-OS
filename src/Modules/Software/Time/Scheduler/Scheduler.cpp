...
Scheduler::Scheduler(ModuleController& controller)
    : Module(controller, "schedule", "Scheduler", "Scheduled CLI executions", true, false, true)
{
            ...
            while (std::getline(cmd_stream, sub_cmd, '$')) {
                ...
                if (!exec_cmd.empty()) {
                    ...
                    DBG_PRINTF(Scheduler, "loop(): executing command for schedule id=%u: %s\n",
                               (unsigned)sched.sid, exec_cmd.c_str());
...
bool Scheduler::add_internal(const std::string& config, uint8_t forced_sid, bool save_to_nvs) {
    DBG_PRINTF(Scheduler, "add_internal(): config='%s', forced_id=%u\n", config.c_str(), (unsigned)forced_sid);
    ...
    if (next_id == 0) {
        ...
        for (const auto& s : schedules) {
            if (s.id >= next_sid) next_id = s.id + 1;
        }
    }

    DBG_PRINTF(Scheduler, "add_internal(): assigning id=%u day=%u start=%u end=%u color='%s'\n",
               (unsigned)next_sid, (unsigned)day, (unsigned)start, (unsigned)end, color_str.c_str());

    schedules.push_back({next_sid, start, end, day, color_str, cmds});
    ...
    if (save_to_nvs) {
        nvs_save_config(next_sid, config);
        nvs_save_active_ids();
    }
    return true;
}

void Scheduler::remove(uint8_t sid) {
...
void Scheduler::load_from_nvs() {
    ...
    schedules.clear();
    std::string ids_str = controller.nvs.read<std::string>(sid, "ids");
    ...
    for (const auto& id_sv : ids) {
        ...
        std::string cfg = controller.nvs.read<std::string>(sid, "cfg_" + id_str);
        if (!cfg.empty()) {
            ...
            add_internal(cfg, parsed_sid, false);
        }
    }
...
void Scheduler::nvs_save_active_ids() {
    std::string ids_str;
    for (size_t i = 0; i < schedules.size(); ++i) {
        ids_str += std::to_string(schedules[i].sid);
        if (i < schedules.size() - 1) ids_str += ",";
    }
    DBG_PRINTF(Scheduler, "nvs_save_active_ids(): writing ids='%s'\n", ids_str.c_str());
    controller.nvs.write<std::string>(sid, "ids", ids_str);
}

void Scheduler::nvs_save_config(uint8_t sid, const std::string& cfg) {
    DBG_PRINTF(Scheduler, "nvs_save_config(): id=%u cfg='%s'\n", (unsigned)sid, cfg.c_str());
    controller.nvs.write<std::string>(sid, "cfg_" + std::to_string(sid), cfg);
}

void Scheduler::nvs_delete_config(uint8_t sid) {
    DBG_PRINTF(Scheduler, "nvs_delete_config(): id=%u\n", (unsigned)sid);
    controller.nvs.remove(sid, "cfg_" + std::to_string(sid));
}

void Scheduler::nvs_clear_all() {
    DBG_PRINTF(Scheduler, "nvs_clear_all(): clearing %u schedules.\n", (unsigned)schedules.size());
    for (const auto& sched : schedules) nvs_delete_config(sched.sid);
    controller.nvs.remove(sid, "ids");
