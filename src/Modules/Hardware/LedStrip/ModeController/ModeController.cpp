...
// src/Modules/Hardware/LedStrip/ModeController/ModeController.cpp
...
bool ModeController::set_mode_param(std::string_view key,
                                    uint16_t value) {
...
bool ModeController::adj_mode_param(std::string_view key,
                                    int32_t value_delta) {
...
std::array<uint8_t, 3> ModeController::get_rgb() const { return current_mode->get_rgb(); }

uint8_t ModeController::get_current_mode_id() const { return current_mode->get_id(); }

std::string_view ModeController::get_current_mode_name() const { return current_mode->get_name(); }

std::vector<ModeParam> ModeController::get_current_mode_params() const { return current_mode->get_params(); }

const ModeConfig& ModeController::get_current_mode_config() const { return current_mode->get_config(); }
...
std::string ModeController::make_nvs_param_key(uint8_t mode_id,
                                               std::string_view param_key) const {
    return "m:" + std::to_string(mode_id) + ":" + std::string(param_key);
}
uint16_t ModeController::normalize_mode_param_value(std::string_view key,
                                                    int32_t value) const {
    for (const auto& param : current_mode->get_params()) {
        if (param.key != key) {
            continue;
