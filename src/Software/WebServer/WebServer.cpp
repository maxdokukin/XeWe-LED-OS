...
<body>
  ...
  <script>
    ...
    function setOnline() {
      clearTimeout(offlineTimer); clearTimeout(reloadTimer);
    ...
    window.addEventListener('load', ()=>{
      ...
      document.getElementById('mode').addEventListener('change',
        ()=>send('mode_id',document.getElementById('mode').value));
      document.getElementById('btnOn').addEventListener('click',
        ()=>{ send('state','1'); updateButtons(true); });
      document.getElementById('btnOff').addEventListener('click',
        ()=>{ send('state','0'); updateButtons(false); });

      // Create Shortcut: go to instruction page
      document.getElementById('btnShortcut').addEventListener('click', () => {
        const params = new URLSearchParams({
          ...
          no_redirect:'1'                // <<< flag to skip redirect on this first visit
        });
...
// Instruction page for `/set_state`
...
<body>
  ...
  <script>
    ...
    // single function to apply everything
    function applyState() {
      ['color','brightness','state','mode_id'].forEach(k => {
    ...
    // on initial click → apply but stay on this page
    if (!noRedirect) {
      console.log('Initial shortcut‐prep load, applying state without redirect');
      applyState();
...
void WebServer::begin() {
  ...
  server_.on("/set_state", HTTP_GET, [this](auto* req){ serve_set_state_page(req);});
...
void WebServer::serve_set_state_page(AsyncWebServerRequest* req) {
  DBG_PRINTF(WebServer, "serve_set_state_page(): url=%s\n", req->url().c_str());
  req->send_P(200, "text/html", SET_STATE_HTML);
  DBG_PRINTLN(WebServer, "serve_set_state_page(): instruction page sent");
}
...
void WebServer::update_state_payload(const char* field) {
  ...
  if (payload_len_ >= kBufSize) {
    ...
    DBG_PRINTLN(WebServer, "update_state_payload: truncated");
  }
