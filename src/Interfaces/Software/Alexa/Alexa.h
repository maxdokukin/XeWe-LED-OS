...
// Now uses a NON-NULL reference to an existing WebServer.
struct AlexaConfig : public ModuleConfig {
    explicit AlexaConfig(WebServer& srv) : server(srv) {}
    WebServer& server;                                // REQUIRED: running WebServer instance (non-null)
};
