...
// styles.css (minified, functionally identical to the prototype)
...
// script.js (minified, feature-parity with the prototype)
...
// Modes (JSON) — backend-provided in production; seeded here for demo/parity with Flask
...
// -----------------------------
// Utility: build no-cache response
// -----------------------------
...
: Interface(controller_ref, "web", "web", false, false, true) // mirror Nvs ctor flags pattern
{
...
void Web::begin(const ModuleConfig& cfg) {
    ...
    const WebConfig* wcfg = dynamic_cast<const WebConfig*>(&cfg);
    port_ = wcfg ? wcfg->port : 80;

    server_ = new AsyncWebServer(port_);
    events_ = new AsyncEventSource("/events");

    // Optional: clients can listen to /events for push updates
    ...
    // Initialize derived HSV from the current color
...
void Web::loop() {
    // Nothing required; AsyncWebServer handles everything in background.
}

void Web::reset(bool verbose) {
    DBG_PRINTLN(Web, "reset(): Reinitializing internal state to defaults.");
    color_      = {255, 0, 0};
    brightness_ = 128;
    state_      = 255;
    mode_       = 0;
...
// -----------------------------
// Web: sync_* (called by controller when *system* changes)
// Keep internal cache aligned + push (optional).
// -----------------------------
...
void Web::setup_routes_() {
    ...
    // POST /api/update with JSON body
...
// -----------------------------
// Private: asset senders
// -----------------------------
void Web::send_index_(AsyncWebServerRequest* req) {
    AsyncWebServerResponse* res = req->beginResponse_P(200, "text/html; charset=utf-8", INDEX_HTML, sizeof(INDEX_HTML));
    add_no_cache(req, res);
    req->send(res);
}
void Web::send_css_(AsyncWebServerRequest* req) {
    AsyncWebServerResponse* res = req->beginResponse_P(200, "text/css; charset=utf-8", STYLES_CSS, sizeof(STYLES_CSS));
    add_no_cache(req, res);
    req->send(res);
}
void Web::send_js_(AsyncWebServerRequest* req) {
    AsyncWebServerResponse* res = req->beginResponse_P(200, "application/javascript; charset=utf-8", SCRIPT_JS, sizeof(SCRIPT_JS));
    add_no_cache(req, res);
    req->send(res);
}
...
void Web::send_modes_json_(AsyncWebServerRequest* req) {
    AsyncWebServerResponse* res = req->beginResponse_P(200, "application/json", MODES_JSON, sizeof(MODES_JSON));
    add_no_cache(req, res);
    req->send(res);
}
...
void Web::handle_update_body_(AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t index, size_t total) {
    // Accumulate full body (AsyncWebServer guarantees order)
    static String body;
    if (index == 0) body = "";
    body.reserve(total);
    ...
    StaticJsonDocument<384> doc; // sufficient for our fields
    ...
    // 2) MODE (validate 0..255; in real setup validate against controller's mode table)
    ...
    // 3) LENGTH (wire 0..255, controller takes uint16_t)
    ...
    // 4) COLOR (takes precedence; derive hue/brightness from it)
    if (doc.containsKey("color")) {
        ...
        if (c && c.size() == 3) {
            ...
            // Derive HSV from RGB; keep S=derived, V->brightness_
            uint8_t h, s, v;
    ...
    // 5) HUE (if provided, recompute color from hue + current brightness)
    ...
    // 6) BRIGHTNESS (update V; recompute color along current hue)
    ...
    // Respond with canonical full state (0–255 everywhere on the wire)
...
void Web::rgb255_to_hsv_(uint8_t r, uint8_t g, uint8_t b, uint8_t& h255, uint8_t& s255, uint8_t& v255) {
    ...
    float mx = max(max(rf, gf), bf);
    float mn = min(min(rf, gf), bf);
    float d  = mx - mn;
...
void Web::normalize_power_() {
    state_ = state_ >= 128 ? 255 : 0;
}
